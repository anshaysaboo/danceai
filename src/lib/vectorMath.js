//GLOBAL VARIABLES
let subVectorKeypointsSameSide = [["wrist", "elbow"], ["elbow", "shoulder"], ["shoulder", "hip"], ["hip", "knee"], ["knee", "ankle"]];
...
function getAngleBetweenVectors(v1, v2) {//helper
    ...
    if (Math.abs(numerator - denominator)/ Math.min(Math.abs(numerator), denominator) < differencePercentThreshold) {
...
function getAngles(liveData, referenceData){ //returns angles of liveData relative to referenceData in form of side, point1, point2: angle, color -> ex for perfect no angle which will be green: left shoulder elbow: 0, [120, 100, 50] 
    angles = {};
    const liveDataDict = extractKeypoints3D(liveData).reduce(function (map, keypoint) {
      map[keypoint.name] = keypoint;
      return map;
    }, {});
    const referenceDataDict = extractKeypoints3D(referenceData).reduce(function(map, keypoint) {
        map[keypoint.name] = keypoint;
        return map;
      }, {});
    const leftRightPrepends = ["left", "right"];
    allKeypoints = [subVectorKeypointsOppositeSide, subVectorKeypointsSameSide];
    for (let z = 0; z < allKeypoints.length; z++){
        ...
        for (let i = 0; i < subVectorKeypoints.length; i++) {
            ...
            if (z == 0){//we are going through opposite side keypoints only (left, right)
                numOfSides = 1;
            }
            for (let j = 0; j < numOfSides; j++) {   
                let side1 = leftRightPrepends[j];   
                let side2 = leftRightPrepends[j];
                if (z==0){//opposite side keypoints
                ...
                let anglesName = flippedSideName + subVectorKeypoints[i][0] + '-' + flippedSideName + subVectorKeypoints[i][1] 
                if (z==0){
                    anglesName = side1 + "_" + subVectorKeypoints[i][0] + '-' + side2+ "_" + subVectorKeypoints[i][1];
                }
                if (liveVector.score > scoreThreshold && referenceVector.score > scoreThreshold) {
                    angle = getAngleBetweenVectors(liveVector, referenceVector);
...
function getColors(angles){ //returns colors of angles in form of side, point1, point2: color -> ex for perfect no angle which will be green: left shoulder elbow: [120, 100, 50]
    colors = {};
...
function getColorForAngleDifference(angle){ //returns color of vector in HSL
    H = Math.max(120 - angle*angleDegreeToHue, 0); //basically yellow if 30 degrees off red if 60 degrees off
    S = 100;
    L = 50;
    return [H, S, L];
}