...
export default function Home() {
  ...
  return (
    <main
      className={`h-screen w-screen bg-yellow flex flex-col ${inter.className}`}
      id="background"
    >
      <div className="w-full flex flex-row-reverse h-full">
        <div
          id="card"
          className="rounded-2xl bg-transparent h-5/6 w-1/2 my-auto flex flex-col"
        >
          ...
          <input
            className=" w-2/3 mx-auto bg-transparent mt-10 border-b border-pink-100 text-pink-100 placeholder-pink-50 appearance-none focus:outline-none"
            type="text"
          ...
          />
          <button
            className="block border px-4 py-2 w-1/3 mt-5 mx-auto hover:bg-pink-100 hover:border-main hover:text-main text-sm rounded-full bg-main text-pink-100 border-pink-100 transition-colors"
            onClick={() => sendURL()}
          >
            Use Video
          </button>
          <h2 className="text-2xl text-pink-100 mx-auto mt-16 font-semibold w-2/3">
